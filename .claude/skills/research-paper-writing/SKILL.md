...
description: Improve academic paper writing quality with clear section structure, paragraph flow, and reviewer-facing presentation. Use when drafting or revising Abstract, Introduction, Literature Review, Method, Results/Analysis, Discussion, or Conclusion; polishing figures/tables; checking claim-support alignment; or performing self-review before submission. Supports both empirical ML-style papers and management science/social science IMRaD papers.
---
...
This skill originated from ML/CV/NLP paper writing conventions. When working on **management science, social science, or software engineering** papers, adapt as follows:

### Structural Differences

| ML/CV/NLP Paper | Management Science / SE Paper |
|-----------------|------------------------------|
| Related Work (separate section) | Literature Review (integrated or standalone) |
| Method = model architecture | Method = research design + data + analysis strategy |
| Experiments = benchmarks + ablation | Results = empirical findings + case analysis |
| Metrics = accuracy/mAP/F1 | Metrics = effect sizes, p-values, qualitative coding |
| Pipeline/teaser figure | Conceptual framework / research model diagram |

### Section-Specific Guidance

- **Introduction**: Same funnel structure applies. Replace "technical challenge" with "research gap." The ML introduction templates in `references/` still work if you map "task" → "research question" and "method pipeline" → "research design."
- **Method**: Use `references/method.md` for clarity principles (motivation → design → advantage), but replace "modules/pipeline" with "constructs/variables/procedures." Management science methods must justify: population, sampling, measurement, analysis strategy.
- **Results/Experiments**: Replace `references/experiments.md`'s ablation studies with: robustness checks, alternative specifications, subgroup analysis, sensitivity analysis.
- **Conclusion**: The ML conclusion guide applies directly — both domains value honest limitations and specific future work.

### Universal Principles (apply to all domains)

The Global Principles, Paragraph Clarity Check, Output Contract, and Paper Review sections below apply to any academic paper regardless of field.
